<h1 class="h3 mb-4 text-gray-800"><?= $title ?> <small style="font-size: 15px;"><?= $judul ?></small></h1>


<div class="row">
	<div class="col-lg-12">
		<div class="card shadow mb-4">
        <div class="card-header py-3 bg-gray-900">
         <!-- <a href="<?= base_url('admin/kelas/add') ?>" class="btn bg-gradient-primary text-white btn-circle btn-sm">
              <i class="fas fa-plus"></i>
          </a> -->
        </div>
        <div class="card-body">

            <?= form_open() ?>
            <div class="row">
                <div class="col-lg-4  col-md-5 col-xs-12">
                  <input type="date" name="tanggal1" class="form-control" required value="<?= set_value('tanggal1') ? set_value('tanggal1') : date('Y-m-d') ?>">
                </div>
                <div class="col-lg-5  col-md-5 col-xs-12">
                  <input type="date" name="tanggal2" class="form-control" required value="<?= set_value('tanggal2') ? set_value('tanggal2') : date('Y-m-d') ?>">
                </div>
                 <div class="col-lg-2  col-md-5 col-xs-12">
                  <select name="type_pemasukan" class="form-control" required>
                    <?php if (!set_value('type_pemasukan')): ?>
                    <option value="">-- type --</option>
                    <?php endif ?>
                    <option value="spp" <?= set_value('type_pemasukan') == 'spp' ? 'selected' : '' ?>>Spp</option>
                    <option value="baju seragam" <?= set_value('type_pemasukan') == 'baju seragam' ? 'selected' : '' ?>>Baju Seragam</option>
                    <option value="biaya lain" <?= set_value('type_pemasukan') == 'biaya lain' ? 'selected' : '' ?>>Biaya Lain</option>
                  </select>
                </div>
                <div class="col-lg-1  col-md-2 col-xs-12">
                  <button class="btn bg-gray-900 text-white" type="submit" name="cari"><i class="fa fa-search" aria-hidden="true"></i> </button>
                </div>
            </div>
            <?= form_close(); ?>

            <div class="row">
              <div class="col-lg-12">
                <hr class="sidebar-divider">
              </div>
            </div>


            <?php if (@$pemasukan): ?>
              <div class="row" style="margin-top: 20px;" id="loading-data-pemasukan">
                <div class="col-lg-12 text-center"><h4><i class="fa fa-spinner fa-spin"></i> Loading</h4></div>
              </div>
              <div class="row" style="margin-top: 20px; display: none;" id="data-pemasukan">
                <div class="col-lg-12">
                  <table cellpadding="5">
                    <tr>
                      <th>Tanggal</th>
                      <td>:</td>
                      <td><?= $this->lib->date_indo($_POST['tanggal1']).' - '.$this->lib->date_indo($_POST['tanggal2']) ?></td>
                    </tr>
                    <tr>
                      <th>Total</th>
                      <td>:</td>
                      <td><?= count($pemasukan) ?> data</td>
                    </tr>
                    <tr>
                      <th>Type Pemasukan</th>
                      <td>:</td>
                      <td><?= ucfirst($_POST['type_pemasukan']) ?></td>
                    </tr>
                  </table>
                </div>
                <div class="col-lg-12"><br><br>
                  <a href="<?= base_url('admin/laporan/print_pemasukan/').$_POST['tanggal1'].'/'.$_POST['tanggal2'].'/'.$_POST['type_pemasukan'] ?>" class="btn btn-default btn-xs pull-right bg-gray-500 text-white" target="_blank"><i class="fas fa-print"></i> Print</a>
                  <br><br>
                  <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover">
                      <thead>
                        <tr>
                          <th class="text-center">No</th>
                          <th>Tanggal Pemasukan</th>
                          <th>Type Pemasukan</th>
                          <th>Jumlah</th>
                          <th>Operator</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php $total_pemasukan=0;$i=1;foreach ($pemasukan as $key): ?>
                        <tr>
                          <td class="text-center"><?= $i ?> .</td>
                          <td><?= $this->lib->date_indo($key->tanggal_pemasukan) ?></td>
                          <td><?= ucfirst($key->type_pemasukan) ?></td>
                          <td>Rp. <?= number_format($key->jumlah_pemasukan,0, ',', '.') ?>,-</td>
                          <td><?= $key->operator ?></td>
                        </tr>
                        <?php $total_pemasukan+=$key->jumlah_pemasukan;$i++;endforeach ?>
                      </tbody>
                      <tfoot>
                        <tr>
                          <td colspan="4" class="text-center" >Jumlah Total </td>
                          <td>Rp. <?= number_format($total_pemasukan,0,',','.'); ?>,-</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
                </div>
              </div>
              <script>
              $(document).ready(function(){
                $('#loading-data-pemasukan').delay(3000).hide('', function(){
                  $('#data-pemasukan').show('slow');
                });
                
              })
             </script>
            <?php endif ?>

            <?php if (@$messageError): ?>
            <div class="row" style="margin-top: 20px;" id="loading-data-pemasukan">
              <div class="col-lg-12 text-center"><h4><i class="fa fa-spinner fa-spin"></i> Loading</h4></div>
            </div>
            <div class="row">
              <div class="col-lg-12">
                <div class="alert alert-danger alert-message" data-dismiss="alert" style="display: none;">
                  Laporan pemasukkan dengan type pembayaran <b><?= ucfirst($_POST['type_pemasukan']) ?></b> , pada tanggal <b><?= $this->lib->date_indo($_POST['tanggal1']).' - '.$this->lib->date_indo($_POST['tanggal2']) ?></b> tidak ditemukan !
                </div>
              </div>
            </div>
            <script>
              $(document).ready(function(){
                $('#loading-data-pemasukan').delay(3000).hide('', function(){
                  $('.alert-message').show('slow', function(){
                    $('.alert-message').delay(8000).hide('slow');
                  });
                });
              })
            </script>
            <?php endif ?>

          



        </div>
        <div class="card-footer bg-gray-900">
          
        </div>
    </div>
	</div>
</div>